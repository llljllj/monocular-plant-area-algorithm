...
from sam3.visualization_utils import plot_results # 从 sam3_utils 模块导入 plot_results 函数，用于可视化结果
...
class restore_algorithm: # 定义一个名为 restore_algorithm 的类，封装视角还原和面积计算的算法
    ...
    def contour_maskarea_world(contour_px, H_world_from_img, precision="float32"): # 定义方法，将图像中的轮廓像素坐标转换为真实物理世界坐标，并计算其在物理世界中的现实面积
        dtype_np = np.float16 if precision == "float16" else np.float32 # 根据要求使用 ifelse 控制数据类型
        pts = contour_px.reshape(-1, 2).astype(dtype_np) # 将输入的轮廓数组拉平为 Nx2 的形状，并转化为指定的 NumPy 浮点型数组，代表 (x, y) 像素坐标
        ones = np.ones((pts.shape[0],1), dtype=dtype_np) # 创建一个形状为 Nx1 的全 1 数组，作为齐次坐标的第三项
    ...
    def get_max_contour(self, inference_state): # 辅助方法：从字典提取 mask 转换为 cv2 的轮廓坐标
        max_mask, _ = self.model.find_maxmask(inference_state)
        if max_mask is None:
            return np.array([])
        
        # 判断掩码是由 GPU tensor 返回还是纯 numpy 返回，转换为 numpy uint8 才能供 cv2 提取轮廓
        if hasattr(max_mask, 'cpu'):
            mask_np = max_mask.cpu().numpy().astype(np.uint8)
        else:
            mask_np = max_mask.astype(np.uint8)

        # 【修复】去掉可能存在的多余维度 (例如从 1xHxW 变为 HxW)
        # cv2.findContours 只能接收严格的二维 (2D) 数组，如果通道数 > 2 就会报 _src.dims() <= 2 的错误
        mask_np = np.squeeze(mask_np)

        contours, _ = cv2.findContours(mask_np, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if len(contours) > 0:
            return max(contours, key=cv2.contourArea) # 返回连通域面积最大的那一个多边形轮廓点集
        return np.array([])

    def reailty_maskarea_restroe(self, first_image, second_image, precision="float32"): # 定义一个流水线主调方法：先提取位姿 -> 求算掩码 -> 视角对齐 -> 后获取各自的真实物理掩码面积
        ...
        state1 = self.model.predict(first_image, self.prompt) # 调用外部模型对象，传入第一帧原图和提示语，预测出相关的推理状态字典
        # print("参考图片面积：%f" % self.contour_maskarea_world(self.get_max_contour(state1), H_world_from_img1, precision=precision)[0]) # 计算第一帧图像中掩码轮廓在物理世界下的面积，并打印出来作为参考
        torch.cuda.empty_cache()  # <--- 加一句清空显存缓存，以防止在处理第二张图时显存不足导致的错误
        state2 = self.model.predict(second_image, self.prompt) # 对第二帧执行相同操作，预测推理状态字典
        
        mask_contour1 = self.get_max_contour(state1) # 
        # samer = SAM3Utils(self.model, load_from_HF=False) # 实例化一个 SAM3Utils 对象，加载指定的模型权重文件
        print("第二张图片面积：%f" % self.contour_maskarea_world(mask_contour1, H_world_from_img2, precision=precision)[0]) # 计算第二帧图像中掩码轮廓在物理世界下的面积，并打印出来作为参考

        self.model.visualize(first_image, state1) # 可视化第一张图的轮廓，注意这里第二个参数必须是完整的推理状态 state1，而不是轮廓坐标
        mask_contour2 = self.get_max_contour(state2) # 提取第二张图的轮廓坐标
        print("第二张图片面积：%f" % self.contour_maskarea_world(mask_contour2, H_world_from_img2, precision=precision)[0]) # 计算第二帧图像中掩码轮廓在物理世界下的面积，并打印出来作为参考
        self.model.visualize(second_image, state2) # 可视化第二张图的轮廓，注意这里必须传入 state2
        restore = self.transform_pose(first_image, second_image) # 调用类方法，基于平面映射假定，强行将第二帧的图片视角形变拉扯至第一帧相机的视角维度下

        area1, _ = self.contour_maskarea_world(mask_contour1, H_world_from_img1, precision=precision) # 拿着第一帧得到的提取出来的轮廓数据及它的单应变换矩阵，计算还原出它真实的物理面积
        area2, _ = self.contour_maskarea_world(mask_contour2, H_world_from_img2, precision=precision) # 对第二帧做一样的事情，获取第二帧掩码面积系在物理空间下映射出来的真实大小
