...
class SAM3Utils:
    ...
    def predict(self,image,prompt):
        image_inference_state = self.processor.set_image(image)# 将图像输入到处理器中，set_image函数会将图像转换为模型所需的格式，并将其存储在处理器的内部状态中，以便后续的文本提示和推理操作使用。
    ...
    def visualize(self,image,inference_state):
        img0 = Image.open(image) # 打开输入图像文件，准备进行可视化。这个步骤是为了在后续的结果可视化中使用原始图像，以便将模型的推理结果（如边界框）绘制在原始图像上进行展示。
